		...
		SubShader
		{
			...
			CGPROGRAM
				...
				fixed4 frag(v2f IN) : SV_Target
				{
					...
					half4 fill = tex2D(_FillTex, IN.texcoord);
					half m = tex2D(_Mask, IN.texcoord).r;
					...
					if (IN.texcoord.y > _amount + 0.125) {
						result = color;
					}
					else {
						result = (color * (1 - m)) + (lerp(fill, fill * fixed4(0, 0, 0, 1), 1 - _amount) * m);
					}
